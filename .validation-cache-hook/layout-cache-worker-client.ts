...
export class LayoutCacheWorkerClient {
  ...
  private readonly listeners = new Set<(event: CacheWorkerEvent) => void>();

  constructor() {
    this.worker = new Worker(new URL("./layout-cache.worker.ts", import.meta.url), { type: "module" });
    this.worker.onmessage = event => this.handleMessage(event.data as CacheWorkerEvent);
  ...
  subscribe(listener: (event: CacheWorkerEvent) => void) {
  ...
  private handleMessage(message: CacheWorkerEvent) {
