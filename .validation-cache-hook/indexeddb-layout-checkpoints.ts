...
export async function listLayoutCheckpoints() {
  const db = await openLayoutCacheWithSchema();
  try {
...
export async function touchLayoutCheckpoint(layoutKey: string, now = Date.now()) {
  const db = await openLayoutCacheWithSchema();
  try {
    const transaction = db.transaction(STORE_CHECKPOINTS, "readwrite");
    const store = transaction.objectStore(STORE_CHECKPOINTS);
    const checkpoint = await request(store.get(layoutKey)) as LayoutCheckpoint | undefined;
    if (checkpoint) store.put({ ...checkpoint, lastAccessedAt: now });
    await transactionDone(transaction);
  } finally {
    db.close();
  }
